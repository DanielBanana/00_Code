...
@jit
def hybrid_ode_stim(z, t, ode_parameters, augment_parameters):
    ...
    derivative = jnp.array([jnp.array((z[1],)),
                            jnp.array((-kappa*z[0]/mass,)) + model.apply(augment_parameters, z) + jnp.array(1.2*jnp.cos(0.628*t))] ).flatten()
    return derivative
...
@jit
def hybrid_ode(z, t, ode_parameters, augment_parameters):
    '''Calculates the right hand side of the hybrid ODE, where
    the damping term is replaced by the neural network'''
    kappa = ode_parameters[0]
    mu = ode_parameters[1]
    mass = ode_parameters[2]
    derivative = jnp.array([jnp.array((z[1],)),
                            jnp.array((-kappa*z[0]/mass,)) + model.apply(augment_parameters, z)]).flatten()
    return derivative

@jit
def adjoint_ode(adj, z, z_ref, t, ode_parameters, augment_parameters):
    '''Calculates the right hand side of the adjoint system.'''
    df_dz = jax.jacobian(hybrid_ode, argnums=0)(z, t, ode_parameters, augment_parameters)
    dg_dz = jax.grad(g, argnums=0)(z, z_ref, ode_parameters, augment_parameters)
    # d_adj = - df_dz(z, ode_parameters).T @ adj - dg_dz(z, z_ref)
    d_adj = - df_dz.T @ adj - dg_dz
    return d_adj

def g(z, z_ref, ode_parameters, augment_parameters):
...
def J(z, z_ref, ode_parameters, augment_parameters):
    '''Calculates the complete loss of a trajectory w.r.t. a reference trajectory'''
    return np.sum(g(z, z_ref, ode_parameters, augment_parameters))
...
def hybrid_euler(z0, t, ode_parameters, augment_parameters):
    '''Applies forward Euler to the hybrid ODE and returns the trajectory'''
    z = np.zeros((t.shape[0], 2))
    z[0] = z0
    for i in range(len(t)-1):
        dt = t[i+1] - t[i]
        z[i+1] = z[i] + dt * hybrid_ode(z[i], t[i], ode_parameters, augment_parameters)
    return z

def adj_euler(a0, z, z_ref, t, ode_parameters, augment_parameters):
    '''Applies forward Euler to the adjoint ODE and returns the trajectory'''
    a = np.zeros((t.shape[0], 2))
    a[0] = a0
    for i in range(len(t)-1):
        dt = t[i+1] - t[i]
        a[i+1] = a[i] + dt * adjoint_ode(a[i], z[i], z_ref[i], t[i], ode_parameters, augment_parameters)
    return a
...
def function_wrapper(augment_parameters, args):
    ...
    augment_parameters = unravel_pytree(augment_parameters)

    z_ref = f_euler(z0, t, ode_parameters_ref)
    z = hybrid_euler(z0, t, ode_parameters, augment_parameters)
    loss = J(z, z_ref, ode_parameters, augment_parameters)

    a0 = np.array([0, 0])
    adjoint = adj_euler(a0, np.flip(z, axis=0), np.flip(z_ref, axis=0), np.flip(t), ode_parameters, augment_parameters)
    adjoint = np.flip(adjoint, axis=0)

    df_dtheta_at_t = vectorized_df_dtheta_function(z, t, ode_parameters, augment_parameters)
...
augment_parameters = model.init(key2, np.zeros((1, 2)))
# augment_parameters = unfreeze(augment_parameters)
flat_augment_parameters, unravel_pytree = flatten_util.ravel_pytree(augment_parameters)
epoch = 0
...
method = 'BFGS'
res = minimize(function_wrapper, flat_augment_parameters, method=method, jac=True, args=args, options={'maxiter': 1000})

flat_augment_parameters = res['x']
augment_parameters = unravel_pytree(flat_augment_parameters)

print(res)

z_ref = f_euler(z0, t, ode_parameters_ref)
z = hybrid_euler(z0, t, ode_parameters_ref, augment_parameters)
