...
def simulate_custom_input(mu = 8.53):
    ...
    for i in range(len(Tspan)-1):
        ...
        df_dx = np.zeros((2,2))
        for j in range(nx):
            df_dx[:, j] = np.array(fmu.getDirectionalDerivative(vr_derivatives, [vr_states[j]], [1.0]))
        df_dmu = fmu.getDirectionalDerivative(vr_derivatives, [vr_input], [1.0])
    ...
    print(f'Directional Derivative (Jacobian) w.r.t. states calculated at t={Tspan[i]}: {df_dx}')
    print(f'Directional Derivative (Jacobian) w.r.t. parameters calculated at t={Tspan[i]}: {df_dmu}')
    fig = plt.figure()
    ax, ax2 = fig.subplots(2,1)
    ax.plot(Tspan, x_history[0])
    ax2.plot(Tspan, x_history[1])
    plt.show()
    fig.savefig('Van_der_Pol_input_mu.png')
