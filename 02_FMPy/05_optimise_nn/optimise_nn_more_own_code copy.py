...
# def hybrid_forward(x, fmu_model, augment_model):
#     u = augment_model(x)
#     dx, y, dfdx, dfdu = fmu_model(u, x)
#     return dx, y, dfdx, dfdu

    # ode/hybrid_ode
def simulate(x0, t, fmu_model, augment_model):
    ...
    # X = torch.empty(n_steps, len(fmu_model._state_reference_numbers), requires_grad=True)
    ...
    if fmu_model.training:
        ans, dfdz, dfdz, t = HybridModelFunction.apply(x0, t, fmu_model, augment_model)
    else:
        ans = HybridModelFunction.apply(x0, t, fmu_model, augment_model)
    return ans
...
class HybridModelFunction(torch.autograd.Function):
    @staticmethod
    def forward(x0, t, X, fmu_model, augment_model):
        """
        In the forward pass we receive a Tensor containing the input and return
        a Tensor containing the output. ctx is a context object that can be used
        to stash information for backward computation. You can cache arbitrary
        objects for use in the backward pass using the ctx.save_for_backward method.
        """
        X = torch.empty(len(t), len(fmu_model._state_reference_numbers), requires_grad=True)
        X[0] = x0
        x = x0
    ...
    def setup_context(ctx, inputs, outputs):
        x0, t, fmu_model, augment_model = inputs
        if fmu_model.training:
...
if __name__ == '__main__':
    ...
    Tspan = torch.tensor(np.linspace(Tstart, Tend, n_steps))
    solver = 'euler'
    ...
    U = torch.zeros((len(dummy_value), len(Tspan), len(fmu_model._input_reference_numbers)))
    ...
    with torch.no_grad():
        ...
        X = simulate(x0, Tspan, fmu_model, augment_model)
    # Yref = Y.detach()
    Xref = X.detach()
