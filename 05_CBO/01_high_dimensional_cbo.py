...
p_plot = ax.scatter(X[:,0], X[:,1], marker='o', color='black', alpha=0.1)
...
def update(X, batch_centers_old, remainder_set):
    batch_centers_new = []
    ...
    for batch in batches:
        ...
        disturbance_deviation = sigma*np.sqrt(gamma)*deviation_from_center*normal_disturbance
        X[batch] = X[batch] - consensus_term - disturbance_deviation
    ...
    return X, batch_centers_old, remainder_set, break_flag


for k in range(n_steps):
    X, batch_centers_old, remainder_set, break_flag = update(X, batch_centers_old, remainder_set)
    if k%10==0:
        a = np.asarray(batch_centers_old)
        ax.scatter(a[:,0], a[:,1],  marker='*', color='red', alpha = k/n_steps)
