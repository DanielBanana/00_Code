...
sample_output_dimension = 1

# TRAINING AND VALIDATION DATA
# Random points in interval [-5, 5]
training_input = np.random.uniform(-5,5, n_training_samples).reshape(1,-1)
validation_input = np.random.uniform(-5,5, n_validation_samples).reshape(1,-1)
training_output = f(training_input) # + 0.1*np.random.rand(sample_output_dimension, n_training_samples)
validation_output = f(validation_input) # + 0.1*np.random.rand(sample_output_dimension, n_validation_samples)
...
design_of_experiment_values = {'beta': [10],
                               'gamma': [0.1],
                               'sigma': [0.4**0.5],
                               'lambda': [1.0],
                               'layer_size': [40]}

levels = [len(val) for val in design_of_experiment_values.values()]
...
for n_exp, experiment in enumerate(doe):
    start = time.time()
    cbo = CBO(beta=design_of_experiment_values['beta'][int(experiment[0])],
          gamma=design_of_experiment_values['gamma'][int(experiment[1])],
          sigma=design_of_experiment_values['sigma'][int(experiment[2])],
          lambda_=design_of_experiment_values['lambda'][int(experiment[3])],
          eps=eps,
          n_particles=n_particles,
          batch_size=batch_size,
          n_iterations=n_iterations)
    layers = [design_of_experiment_values['layer_size'][int(experiment[4])], 1]
    ...
    experiment_strings.append(f'Experiment {n_exp} - beta: {cbo.beta:3.3f}, gamma: {cbo.gamma:3.3f}, sigma: {cbo.sigma:3.3f}, lambda: {cbo.lambda_:3.3f}, layer size: {layers[0]}')
