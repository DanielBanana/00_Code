...
from cbo_in_python.src.torch_.particle import Particle
from cbo_in_python.src.torch_.cbo import cbo_update, compute_v_alpha
from cbo_in_python.src.torch_.utils import inplace_randn
...
class Optimizer:
    def __init__(self, model, n_particles=10, l=1, alpha=100, sigma=1, dt=0.01, anisotropic=True, eps=1e-2,
                 ...
                 evaluation_strategy='last', fmu=False, residual=False, restore=False):
        """
        ...
        self.model = model
        self._initialize_particles(restore=restore)
    ...
    def cooling_step(self):
        ...
        self.alpha *= 1.001
        self.sigma *= 1.001
        self.sigma = self.sigma0 / np.log2(self.epoch + 2)
    ...
    def _initialize_particles(self, restore=False):
        self.particles = [Particle(self.model, self.fmu, self.residual, restore=restore) for _ in range(self.n_particles)]
