...
def create_nn(model: str, inputs, layers):
    key1, key2 = jrandom.split(jrandom.PRNGKey(np.random.randint(0,100)))
    if model == 'TinyMLP':
        nn = TinyMLP()
    elif model == 'SmallMLP':
        nn = SmallMLP()
    elif model == 'MediumMLP':
        nn = MediumMLP()
    elif model == 'CustomMLP':
        nn = CustomMLP(features=layers)
    else:
        print('Model not supported')
    nn_parameters = nn.init(key1, np.zeros((1, inputs.shape[0])))
    jitted_neural_network = jax.jit(lambda p, x: nn.apply(p, x))
