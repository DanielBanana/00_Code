...
if __name__ == '__main__':
    ...
    parser.add_argument('--script_file', type=str, default='script.py', help='The script for the design of experiments')
    ...
    parser.add_argument('--results_directory_name', type=str, default='DoE',
                        help='The yaml file which contains the DoE arguments for the script')
    ...
    with open('VdP_CBO_DoE.yaml', 'r') as file:
        doc_generator = yaml.safe_load_all(file)
        for arguments in doc_generator:
            pass

    fixed_arguments = arguments['fixed_arguments']
    fixed_arguments['results_file'] = args.results_file
    fixed_arguments['results_directory'] = results_directory
    ...
    for run in range(1, args.n_runs+1):
        for n_exp, exp in enumerate(fullfact_experiments):
            ...
            result = subprocess.run(['python', args.script_file] + fullfact_argument_list, capture_output=True)
        ...
        for result_parameter, v in averaged_results[0].items():
            if type(v) is list or type(v) is tuple:
                ...
                for exp_index, averaged_result in enumerate(averaged_results):
                    ...
                    ax.set_title(result_parameter)
                plt.savefig(f'{result_parameter}_Run-{run}.png')
            elif type(v) is int or type(v) is float:
                fig, ax = plt.subplots(1, 1)
                for exp_index, averaged_result in enumerate(averaged_results):
                    value = averaged_result[result_parameter]
                    ax.bar(exp_index, value)
                    ax.set_ylabel(result_parameter)
                    ax.set_title(result_parameter)
                plt.savefig(f'{result_parameter}_Run-{run}.png')
